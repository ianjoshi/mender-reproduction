...
class MENDER_single(object):
    ...
    def set_MENDER_para(
        ...
        if nn_mode not in ["ring", "radius", "k"]:
            ...
            print(
                "radius: not restricted by spatial technologies, each scale is determined by the graph constructed by radius-nn"
            )
            print(
                "k: not restricted by spatial technologies, each scale is determined by the graph constructed by k-nn"
            )
        ...
        if count_rep not in ["s", "a"]:
            ...
            print(
                "a: accumulate the cell type frequencies for each single scale and all subordinate scales"
            )
            return
        ...
        for i in range(I.shape[0]):
            if self.verbose == 2:
                if i % 1000 == 0:
                    ...
                    print(
                        "{0} MEs,time cost {1} s, {2} MEs, {3}s left".format(
                            i,
                            time_end - time_start,
                            I.shape[0] - i,
                            (I.shape[0] - i) * (time_end - time_start) / 1000,
                        )
                    )
                    time_start = time.time()
            ...
            cur_cls_unique, cur_cls_count = np.unique(
                cur_neighbors_cls, return_counts=1
            )  # counting for each cluster
            cur_cls_idx = [
                np.where(ME_var_names_np_unique == c)[0][0] for c in cur_cls_unique
            ]  # c is string
    ...
    def ring_representation(self, group_norm=False):
        ...
        ME_X_prev = np.zeros(
            shape=(cls_array.shape[0], ME_var_names_np_unique.shape[0])
        )
        ...
        for cur_scale in range(self.n_scales):
            ...
            if self.count_rep == "a":
                ...
                ME_X_prev = ME_X
            print(
                f"scale {cur_scale}, median #cells per ring (r={self.nn_para}):",
                np.median(np.sum(cur_X, axis=1)),
            )

            self.adata.obsm[cur_ME_key] = cur_X.copy()
            if group_norm:
                self.adata.obsm[cur_ME_key] = self.adata.obsm[cur_ME_key] / np.sum(
                    self.adata.obsm[cur_ME_key], axis=1, keepdims=True
                )
                self.adata.obsm[cur_ME_key] = np.nan_to_num(
                    self.adata.obsm[cur_ME_key], 0
                )
    ...
    def radius_representation(self, group_norm=False):
        ...
        for i in range(self.n_scales):
            ...
            sq.gr.spatial_neighbors(
                self.adata,
                coord_type="generic",
                radius=self.nn_para * (cur_scale + 1),
                set_diag=self.include_self,
            )
            I = self.adata.obsp["spatial_connectivities"]
            ME_X = np.zeros(shape=(cls_array.shape[0], ME_var_names_np_unique.shape[0]))
            ...
            if self.count_rep == "a":
                ...
                ME_X_prev = ME_X
            print(
                f"scale {cur_scale}, median #cells per radius (r={self.nn_para}):",
                np.median(np.sum(cur_X, axis=1)),
            )
    ...
    def preprocess_adata_MENDER(self, mode=0, neighbor=True):
        if (
            hasattr(self, "is_adata_MENDER_preprocess") == False
            or self.is_adata_MENDER_preprocess == False
        ):
            if mode == 0:
    ...
    def run_clustering_normal(
        self, target_k, run_umap=False, if_reprocess=True, mode=0
    ):
        if if_reprocess:
            self.preprocess_adata_MENDER(mode)
        if run_umap:
            sc.tl.umap(self.adata_MENDER)
            self.adata_MENDER.obsm["X_MENDERMAP2D"] = self.adata_MENDER.obsm[
                "X_umap"
            ].copy()

        if target_k > 0:
            res = res_search(
                self.adata_MENDER, target_k=target_k, random_state=self.random_seed
            )
            sc.tl.leiden(
                self.adata_MENDER,
                resolution=res,
                key_added=f"MENDER_leiden_k{target_k}",
                random_state=self.random_seed,
            )
            self.adata_MENDER.obs["MENDER"] = self.adata_MENDER.obs[
                f"MENDER_leiden_k{target_k}"
            ].copy()
        elif target_k < 0:
            res = -target_k
            sc.tl.leiden(
                self.adata_MENDER,
                resolution=res,
                key_added=f"MENDER_leiden_res{res}",
                random_state=self.random_seed,
            )
            self.adata_MENDER.obs["MENDER"] = self.adata_MENDER.obs[
                f"MENDER_leiden_res{res}"
            ].copy()
...
class MENDER(object):
    ...
    def run_representation(self, group_norm=False):
        ...
        for i in range(len(self.batch_list)):
            ...
            print(
                f"total batch: {len(self.batch_list)}, running batch {cur_batch_name}"
            )

            cur_MENDER = MENDER_single(
                cur_batch_adata,
                ct_obs=self.ct_obs,
    ...
    def mp_helper(self, i):
        ...
        cur_MENDER = MENDER_single(
            cur_batch_adata, ct_obs=self.ct_obs, verbose=self.verbose
        )
        cur_MENDER.set_MENDER_para(
            nn_mode=self.nn_mode,
            nn_para=self.nn_para,
    ...
    def run_clustering_mclust(self, target_k, run_umap=False):
        ...
        if target_k > 0:
            self.adata_MENDER = STAGATE.mclust_R(
                self.adata_MENDER, used_obsm="X_pca", num_cluster=target_k
            )

            self.adata_MENDER.obs["MENDER"] = self.adata_MENDER.obs[
                f"MENDER_mclust_k{target_k}"
            ].copy()
    ...
    def output_cluster(self, dirname, obs):
        # path = 'a'
        sc.pl.embedding(
            self.adata_MENDER, basis="X_MENDERMAP2D", color=obs, save=f"_umap_{obs}"
        )
        path = dirname
        ...
        for i in range(len(self.batch_list)):
            ...
            ax.axis("equal")
            plt.savefig(
                f"figures/spatial_{path}/{cur_batch}.png",
                dpi=200,
                bbox_inches="tight",
                transparent=True,
            )
    ...
    def output_cluster_all(self, obs="MENDER", obs_gt="gt"):
        ...
        self.adata_MENDER.obs[self.batch_obs] = self.adata_MENDER.obs[
            self.batch_obs
        ].astype("category")
