...
def res_search(
    adata, target_k=7, res_start=0.1, res_step=0.05, res_epochs=10, random_state=666
):
    ...
    while old_k != target_k:
        ...
        sc.tl.leiden(
            adata, resolution=res + res_step * old_sign, random_state=random_state
        )
...
def markerFC(adata_valid, marker_list, sdm_key):
    ...
    for marker in marker_list:
        ...
        for sdm in sdm_unique:
            mean_exp_list.append(
                np.mean(adata_valid[adata_valid.obs[sdm_key] == sdm][:, marker].X)
            )
        ...
        other_sdm_value = np.mean(
            adata_valid[adata_valid.obs[sdm_key] != sdm_unique[max_sdm_idx]][
                :, marker
            ].X
        )
...
def mclust_R(adata, num_cluster, modelNames="EEE", used_obsm="X_pca", random_seed=666):
    ...
    adata.obs[f"MS_mclust_k{num_cluster}"] = adata.obs[
        f"MS_mclust_k{num_cluster}"
    ].astype("int")
    adata.obs[f"MS_mclust_k{num_cluster}"] = adata.obs[
        f"MS_mclust_k{num_cluster}"
    ].astype("category")
...
def CNC(adata_raw, leiden_res, k, is_preprocessed=False):
    ...
    for i in range(I.shape[0]):
        ...
        cur_cls_idx = [
            np.where(ct_array == c)[0][0] for c in cur_cls_unique
        ]  # c is string
