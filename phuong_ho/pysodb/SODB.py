...
class SODB:
    def __init__(
        self,
        storage_path=None,
        server_address="https://gene.ai.tencent.com/SpatialOmics/api/pysodb",
    ):
        """
    ...
    def list_dataset_by_category(self, biotech_category):
        ...
        return: list of datasets (short name): [str, ...]
        """
        biotech_category_list = set(
            self.data.loc[:, "biotech_category"].values.tolist()
        )
        assert (
            biotech_category in biotech_category_list
        ), 'Unkown biotech category: {}, Available categories are "Spatial Transcriptomics", "Spatial Proteomics", "Spatial Metabolomics", "Spatial Genomics", "Spatial MultiOmics". '.format(
            biotech_category
        )
        return list(
            set(
                self.data[self.data.biotech_category == biotech_category]
                .loc[:, "dataset_name"]
                .values.tolist()
            )
        )
    ...
    def list_experiment_by_dataset(self, dataset_name):
        ...
        if len(self.data[self.data.dataset_name == dataset_name]) <= 0:
            raise Exception(
                "dataset[{}] does not exist. You could get available datasets by calling the function list_dataset.".format(
                    dataset_name
                )
            )
        return (
            self.data[(self.data.dataset_name == dataset_name)]
            .loc[:, "experiment_name"]
            .values.tolist()
        )
    ...
    def load_dataset(self, dataset_name):
        ...
        return {
            en: self.load_experiment(dataset_name, en) for en in experiment_name_list
        }
        ...
        if (
            len(
                self.data[
                    ...
                    & (self.data.experiment_name == experiment_name)
                ]
            )
            <= 0
        ):
            raise Exception(
                "experiment[{}] does not exist in dataset[{}]. You could get available experiments in dataset[{}] by calling the function list_experiment_by_dataset.".format(
                    experiment_name, dataset_name, dataset_name
                )
            )
        tmp_storage_path = os.path.join(
            self.storage_path, dataset_name, experiment_name + ".h5ad"
        )
        if not os.path.exists(tmp_storage_path):
            os.makedirs(os.path.join(self.storage_path, dataset_name), exist_ok=True)
            # download
            print(
                "download experiment[{}] in dataset[{}]".format(
                    experiment_name, dataset_name
                )
            )
            download_url_to_file(
                self.server_address
                + "/download/{}/{}".format(dataset_name, experiment_name),
                tmp_storage_path,
            )
        try:
            print(
                "load experiment[{}] in dataset[{}]".format(
                    experiment_name, dataset_name
                )
            )
            ...
            print(
                "False to load experiment[{}] in dataset[{}], please try again later.".format(
                    experiment_name, dataset_name
                )
            )
