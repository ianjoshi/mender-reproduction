...
class SMENDER_single(object):
    def __init__(self,adata,ct_obs='ct',verbose=0,random_seed=666, ann=None):
    ...
    def mp_helper(self,cur_scale):
        ...
        if self.ann:    
            ...
            raise Exception('Please input a valid ANN object')
...
class SMENDER(object):
    def __init__(self,adata,batch_obs,ct_obs='ct',verbose=0,random_seed=666, ann=None):
        self.adata = adata
        self.batch_obs = batch_obs
        self.ct_obs = ct_obs
        self.verbose = verbose
        self.random_seed = random_seed
        self.ann_class = ann  # Store the class, not the instance
        self.ct_unique = np.array(self.adata.obs[self.ct_obs].cat.categories)
        self.nn_mode = None
        self.nn_para = None
        self.count_rep = None
    ...
    def mp_helper(self,i):
        ...
        cur_batch_adata = self.adata_list[i]
        print(f'total batch: {len(self.batch_list)}, running batch {cur_batch_name}')
        # Instantiate ann inside the subprocess
        ann_instance = self.ann_class() if self.ann_class is not None else None
        cur_MENDER = SMENDER_single(cur_batch_adata,ct_obs=self.ct_obs,verbose=self.verbose,ann=ann_instance)
        cur_MENDER.set_MENDER_para(
            nn_mode=self.nn_mode,
            nn_para=self.nn_para,
            count_rep=self.count_rep,
    ...
    def estimate_radius(self):
        for i in range(len(self.batch_list)):
            cur_batch_name = self.batch_list[i]
            cur_batch_adata = self.adata_list[i]
            cur_MENDER = SMENDER_single(cur_batch_adata,ct_obs=self.ct_obs,verbose=self.verbose)
